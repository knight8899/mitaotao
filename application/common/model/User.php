...
class User extends Base
{
    ...
    public function register($param)
    {
        ...
        if (empty($param['codes'])){
            ...
            $uid = Db::table('mac_user')->where('user_codes',$param['codes'])->value('user_id');
        }
		...
		$fields = [];
        $codes = $this->create_invite_code();
        while(true) {
            $ros = Db::table('mac_user')->where('user_codes',$codes)->find();
            if (empty($ros)) {
                break;
            }
        }

        $fields['user_codes'] = $codes;
